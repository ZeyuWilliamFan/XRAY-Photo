    ...
    lazy var managedObjectContext: NSManagedObjectContext = {
        if let modelURL = NSBundle.mainBundle().URLForResource("DataModel", withExtension: "momd") {
            if let model = NSManagedObjectModel(contentsOfURL: modelURL) {
                ...
                println("\(storeURL)")
                var error: NSError?
                if let store = coordinator.addPersistentStoreWithType(  NSSQLiteStoreType, configuration: nil,
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        let navigationController = window!.rootViewController as! UINavigationController
            ...
            if let VC = navigationController.viewControllers {
                let photoMainViewController = VC[0] as! PhotoMain
                photoMainViewController.managedObjectContext = managedObjectContext
            }
